...
- **Bump the version every commit.** Increment `version` in `package.json` as part of each commit, so every commit is a distinct running build and the status bar's version badge (`v<version>`, beside the vault's file count) names the build that is live. A patch bump for fixes, refactors, and docs; a minor bump for a new user-facing capability. The version is build identity, not a release schedule.
- **Dev servers leave no survivors.** A browser check starts a server, and this machine is Windows: `pkill -f vite` from Git Bash signals nothing (it exits 0 having killed no Windows-native `node.exe`), so a server started for a check keeps running, silently serves the next check, and makes a fixed port fail with "already in use". Start browser-check servers with `npm run dev:test` (`vite --port 0`, so the OS picks a free port and the log names it), confirm the log says `ready in` before trusting what the check measured, and sweep the leftovers with `npm run kill:dev` when done. A smoke test that ran against a stale server is not evidence.
