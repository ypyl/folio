...
- Folio's reference forms are `[[Page]]`, `#word`, and `#[[Page]]`. They are syntactic variants of the same thing, not features.
...
- Parser, index, and editor handle one concept (page references) in three lexical forms.
...
- Existing vaults using other conventions are read as Markdown text, but their conventions are not features.